...
func (s *service) Join(ctx context.Context, challengeID, userID, idem string) error {
	...
	return db.InTx(ctx, s.d.Pool, func(tx pgx.Tx) error {
		ch, err := s.d.Store.Challenges.Get(ctx, challengeID)
		if err != nil {
			return err
		}
		...
		// 3. Tạo participant.
		if err := s.d.Store.Challenges.AddParticipant(ctx, tx, challengeID, userID, ch.EntryPoints); err != nil {
			return err
		}
		return nil
	})
...
func (s *service) Settle(ctx context.Context, id string) (*SettleResult, error) {
	...
	if len(parts) == 0 {
		_ = s.d.Store.Challenges.SetStatus(ctx, id, "done")
		return &SettleResult{ChallengeID: id, Winners: nil, Refunded: false}, nil
	}
	// Winners: total_steps >= daily*duration.
	threshold := int64(ch.DailyStepsTarget) * int64(ch.DurationDays)
	winners := []string{}
	for _, p := range parts {
		if p.TotalSteps >= threshold {
	...
	err = db.InTx(ctx, s.d.Pool, func(tx pgx.Tx) error {
		for _, w := range winners {
			...
			_ = s.d.Store.Challenges.SetParticipantState(ctx, tx, id, w, "won")
		}
		if hostCut > 0 && ch.HostID != nil {
			_ = s.d.Store.Wallet.AppendEntry(ctx, tx, store.LedgerEntry{
